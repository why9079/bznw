...
class CartController extends BaseController
{
    ...
    public function delCart()
    {
        ...
        // 查找购物车数量
        $user_id = I("user_id"); // 唯一id  类似于 pc 端的session id
        $cart_count = cart_goods_num(0, $user_id);
    ...
    {
        $cart_form_data = str_replace('\'','"',$_POST["cart_form_data"]);// goods_num 购物车商品数量
        ...
        if ($cart_form_data) {
            ...
            foreach ($cart_form_data as $key => $val) {
                ...
                if (($cartList[$cartID]['goods_num'] != $data['goods_num']) || ($cartList[$cartID]['selected'] != $data['selected']))
                    $resultsql[]=M('Cart')->where("id = $cartID")->save($data);
            }
            $result=array('status'=>1,'msg'=>'修改购物车','result'=>$resultsql);
            //$this->assign('select_all', $_POST['select_all']); // 全选框
        }else{
//            $result = $this->cartLogic->cartList($this->user, $unique_id, 1, 0); // 选中的商品
            $result = $this->cartLogic->cartListall(I("user_id"), 1, 0);
        ...
        //$result['print']=print_r($cart_form_data);
        exit(json_encode($result));//json_encode($result)
    }
    ...
    public function cart2()
    {
        ...
        // 购物车商品                    
        $cart_result = $this->cartLogic->cartListapp($user_id, $unique_id, 1, 1); // 获取购物车商品
        // 没有选中的不传递过去
        $cartList = array();
        ...
        // 收货地址
        $addresslist = M('region3')->where(array('user_id'=>$user_id,'default'=>'1'))->select();
//        $c = M('region3')->where(array('user_id'=>$user_id,'default'=>'1'))->count(); // 看看有没默认收货地址        
//        if ((count($addresslist) > 0) && ($c == 0)) // 如果没有设置默认收货地址, 则第一条设置为默认收货地址
//            $addresslist[0]['default'] = 1;
...
//                'couponList' => $couponList, //优惠券列表
                'userInfo' => array('pay_points'=>$usersInfo['pay_points']), // 用户详情
            ));
    ...
    public function cart3()
    {
        ...
        $address = M('region3')->where("address_id = $address_id")->find();
        $order_goods = M('cart')->where("user_id = {$this->user_id} and selected = 1")->select();
        $result = calculate_price($this->user_id, $order_goods, $shipping_code, 0, 0, 0, 0, $pay_points, $user_money, $coupon_id, $couponCode);
        ...
        if ($_REQUEST['act'] == 'submit_order') {
            ...
            $result = $this->cartLogic->addOrder($this->user_id, $address_id, $shipping_code, $invoice_title, $coupon_id, $car_price,$is_app); // 添加订单
            exit(json_encode($result));
        }
