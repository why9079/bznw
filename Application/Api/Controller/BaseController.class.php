...
class BaseController extends Controller {
    ...
    public function _initialize() {
        ...
        // 不参与签名验证的方法
        if(!in_array(strtolower(ACTION_NAME), array('getservertime','getconfig','alipaynotify','goodslist','search','cartList','goodsthumimages')))//,'login'
        {        
        ...
        unset($data['sign']);    // 删除这两个参数再来进行排序
                $json_arr = array('status'=>-1,'msg'=>'签名失败!!!','result'=>''.$local_sign.'/'.$_POST['sign'],'post'=>ksort($data),);
    ...
    public function checkToken(){
        ...
        $check_arr = array(
            ...
            'user'=>array('delorder','upheadpic','uptel','userinfo','updateuserinfo','password','getaddresslist','addaddress','del_address','setdefaultaddress','getcouponlist','getgoodscollect','getorderlist','getorderdetail','cancelorder','orderconfirm','add_comment','account','return_goods_list','return_goods_info','return_goods_status','return_goods','getaddresslistapp','addaddressapp','deladdressapp'), // 需要验证登录的方法 
            'cart'=>array('cart2','cart3','delCart'), // 需要验证登录的方法 
...
//                    if(time() - $this->user['last_login'] > 86400)   //3600                                          
