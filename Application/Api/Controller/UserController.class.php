...
class UserController extends BaseController {
    ...
    public function codelogin(){
        $usertel = I('usertel');
        $unique_id = I("unique_id"); // 唯一id  类似于 pc 端的session id
        $data = $this->userLogic->codelogin($usertel);
        if($data['status'] != 1)
            exit(json_encode($data));        
...
//        }        
    ...
    public function upTel() {
        ...
        $phone=I('phone');
        //检测手机号码是否被注册
        $count = M('users')->where("mobile = $phone")->count();
        if($count>0)
            exit (json_encode (array("status"=>-1,"msg"=>"该手机号码已被注册","result"=>'')));
        M('users')->where("user_id = $user_id")->save(array('mobile'=>$phone));
    ...
    public function upheadpic(){
        ...
        $upload_path = "D:/wwwroot/bznwshop/Public/upload/head_pic/app/"; //上传文件的存放路径
        $name = $file['name'];
        $type = strtolower(substr($name,strrpos($name,'.')+1));
        $picname=$this->user_id.".".$type;
        $url="/Public/upload/head_pic/app/".$picname;
        if(move_uploaded_file($file['tmp_name'],$upload_path.$picname)){
          M('users')->where("user_id = {$this->user_id}")->save(array("head_pic"=>$url));
          exit(json_encode(array('status'=>1,'msg'=>'上传成功','result'=>"http://".$_SERVER["HTTP_HOST"].$url)));
        }else{
          exit(json_encode(array('status'=>-1,'msg'=>'上传失败','result'=>"http://".$_SERVER["HTTP_HOST"].$url)));
        }
    }
    ...
    public function updateUserInfo(){
        if(IS_POST){
            ...
            //exit(json_encode(array('result'=>$_FILES)));
    ...
    public function forgetpass(){
        //$pass = encrypt(I('password'));
        
        $row = M('users')->where("mobile = ".I('mobile'))->save(array('password'=>encrypt(I('password'))));
        if(!$row)
            exit (json_encode (array('status'=>-1,'msg'=>'修改失败','result'=>NULL)));
        exit (json_encode (array('status'=>1,'msg'=>'修改成功','result'=>NULL)));
    }
    /**
     * 获取收货地址
     */
     ...
     * 获取收货地址app
     */
    public function getAddressListapp(){
        $user_id = I('user_id');
        if(!$user_id)
            exit(json_encode(array('status'=>-1,'msg'=>'缺少参数','result'=>'')));
            $address = M('region3')->where(array('user_id'=>$user_id))->select();
        if(!$address)
            exit(json_encode(array('status'=>1,'msg'=>'没有数据','result'=>'')));
        exit(json_encode(array('status'=>1,'msg'=>'获取成功','result'=>$address)));
    }
     ...
     * 添加/编辑地址app
     */
    public function addAddressapp(){
        $user_id = I('user_id');
        $address_id = I('address_id');
        if(!$user_id) exit(json_encode(array('status'=>-1,'msg'=>'缺少参数','result'=>'')));
        $address_id = I('address_id',0);
        $data = $this->userLogic->add_addressapp($user_id,$address_id,I('post.')); // 获取用户信息
        exit(json_encode($data));
    }
    /*
     * 地址删除
...
//        $user_id = I('user_id',0);
     ...
     * 设置默认收货地址app
     */
    public function setDefaultAddressapp(){
        $user_id = I('user_id');
        if(!$user_id) exit(json_encode(array('status'=>-1,'msg'=>'缺少参数','result'=>'')));
        $address_id = I('address_id');
        $data = $this->userLogic->set_defaultapp($user_id,$address_id); // 获取用户信息
        if(!$data)
            exit(json_encode(array('status'=>-1,'msg'=>'操作失败','result'=>'')));
        exit(json_encode(array('status'=>1,'msg'=>'操作成功','result'=>'')));
    }
     ...
     * 删除收货地址
     */
    public function delAddressapp(){
        $user_id = I('user_id');
        $address_id = I('address_id');
        //判断是否为默认收货地址
        $data = M('region3')->where("address_id = $address_id")->find();
        if($data['default']==1)
            exit(json_encode(array('status'=>-1,'msg'=>'默认收货地址不能删除','result'=>'')));
        M('region3')->where("address_id = $address_id")->delete();
            exit(json_encode(array('status'=>1,'msg'=>'删除成功','result'=>'')));
    }
    /*
     * 获取优惠券列表
...
//        $user_id = I('user_id',0);
    ...
    public function getOrderList(){
       // $user_id = I('user_id',0);
        ...
        //$map['user_id'] = $user_id;
        $map = " user_id = {$this->user_id} and is_show = 1 ";        
    ...
    public function delOrder() {
        $order_id = I("id");//获取订单id
        M('order')->where("order_id = $order_id")->save(array("is_show"=>0));
        exit(json_encode(array('status'=>1,'msg'=>'删除成功','result'=>"")));
