...
class GoodsController extends BaseController {
    ...
    public function search(){
    	...
    	if($count > 0)
    	{
                $goods_list = M('goods')->field('goods_id,cat_id,goods_sn,goods_name,original_img,shop_price,comment_count')->where("goods_id in (".  implode(',', $filter_goods_id).")")->order("$sort $sort_asc")->limit($page->firstRow.','.$page->listRows)->select();    		
    		$filter_goods_id2 = get_arr_column($goods_list, 'goods_id');
    	}
    	$goods_category = M('goods_category')->where('is_show=1')->cache(true)->getField('id,name,parent_id,level'); // 键值分类数组
    ...
    public function goodsInfo(){
       ...
       //截取图片
        ...
        preg_match_all($pattern,$goods['goods_content'],$match);
        //print_r($match[1]);
        $goods['goods_content']=$match[1];
        
       //$goods['goods_content'] = htmlspecialchars($goods['goods_content']);
       $goods['original_img'] = SITE_URL.$goods['original_img'];
        $return['goods'] = $goods;
        $return['spec_goods_price']  = M('spec_goods_price')->where("goods_id = $goods_id")->field("key,price,store_count")->select(); // 规格 对应 价格 库存表
        if(empty($return['spec_goods_price'])){$return['spec_goods_price']="";}
