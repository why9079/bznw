...
//            $where .= " and  session_id = '$session_id' ";
     ...
     */
    function cartListall($user = array(), $selected = 0,$mode =0){
        $where = " 1 = 1 ";
        //$where .= " and user_id = $user[user_id] ";
        is_array($user)?$where .= " and user_id = $user[user_id] ":$where .= " and user_id = $user";
        $cartList = M('Cart')->where($where)->select();  // 获取购物车商品 
        //插入商品缩略图
        foreach ($cartList as $k=>$val){
            $where = ' goods_id = '.$cartList[$k]['goods_id'];
            $original_img = M('Goods')->where($where)->select();
            $cartList[$k]['original_img']= $_SERVER['HTTP_HOST'].$original_img[0]['original_img'];
        }
    ...
    function cartListapp($user , $session_id = '', $selected = 0,$mode =0)
    {                   
...
function cart_freight2($shipping_code,$province,$city,$district,$weight)
{
   ...
   return $freight;  
     ...
     */
    public function addOrder($user_id,$address_id,$shipping_code,$invoice_title,$coupon_id = 0,$car_price,$is_app = 0)
    {
        ...
        if($order_count >= 50) 
         ...
         // 0插入订单 order
        //判断是app端地址还是web端地址
        $is_app == 0?$address = M('UserAddress')->where("address_id = $address_id")->find():$address = M('region3')->where("address_id = $address_id")->find();
        ...
        $data = array(
                ...
                'user_id'          =>$user_id, // 用户id
                'consignee'        => $is_app == 0?$address['consignee']:$address['real_name'], // 收货人
                'province'         => $is_app == 0?$address['province']:0,//'省份id',
                'city'             => $is_app == 0?$address['city']:0,//'城市id',
                'district'         => $is_app == 0?$address['district']:0,//'县',
                'twon'             => $is_app == 0?$address['twon']:0,// '街道',
                'address'          => $address['address'],//'详细地址',
                'mobile'           => $is_app == 0?$address['mobile']:$address['real_phone'],//'手机',
                'zipcode'          => $is_app == 0?$address['zipcode']:0,//'邮编',            
                'email'            => $is_app == 0?$address['email']:0,//'邮箱',
                ...
                'is_app' => $is_app,
                'province_app' => $is_app == 1?$address['province']:"0",
                'city_app' => $is_app == 1?$address['city']:0,
        );
        ...
        return array('status'=>1,'msg'=>'提交订单成功','result'=>$order_id,'sql'=>$sql); // 返回新增的订单id        
    }
