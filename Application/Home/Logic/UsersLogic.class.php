...
class UsersLogic extends RelationModel
{
     ...
     * 手机验证码动态登陆
     */
    public function codelogin($username){
        $result = array();
    ...
    public function reg($username,$password,$password2){
     ...
     public function get_info($user_id){
        ...
        if(!$user_info)
         ...
         //$user_info['head_pic']="http://".$_SERVER["HTTP_HOST"].$user_info['head_pic'];
         if(!empty($user['head_pic'])){
                $user['head_pic']="http://".$_SERVER["HTTP_HOST"].$user['head_pic'];
            }else{
                $user['head_pic']="";
...
//        $type = I('get.type',0);
     ...
     * @return array
     */
    public function add_addressapp($user_id,$address_id=0,$data){
        $post = $data;
        //检查手机格式
        if($post['real_name'] == '')
            return array('status'=>-1,'msg'=>'收货人不能为空','result'=>'');
        ...
        if(!$post['address'])
            return array('status'=>-1,'msg'=>'地址不能为空','result'=>'');
        if(!check_mobile($post['real_phone']))
            return array('status'=>-1,'msg'=>'手机号码格式有误','result'=>'');
        //编辑模式
        if($address_id > 0){

            $address = M('region3')->where(array('address_id'=>$address_id,'user_id'=> $user_id))->find();
            if($post['default'] == 1 && $address['default'] != 1)
                M('region3')->where(array('user_id'=>$user_id))->save(array('default'=>0));
            $row = M('region3')->where(array('address_id'=>$address_id,'user_id'=> $user_id))->save($post);
            if(!$row)
                return array('status'=>-1,'msg'=>'操作完成','result'=>'');
            return array('status'=>1,'msg'=>'编辑成功','result'=>'');
        }
        ...
        // 如果目前只有一个收货地址则改为默认收货地址
        $c = M('region3')->where("user_id = {$post['user_id']}")->count();        
        if($c == 0)  $post['default'] = 1;
      
        $address_id = M('region3')->add($post);
        //如果设为默认地址
        $insert_id = M()->getLastInsID();
        $map['user_id'] = $user_id;
        $map['address_id'] = array('neq',$insert_id);
        ...
        if($post['default'] == 1)
            M('region3')->where($map)->save(array('default'=>0));
        if(!$address_id)
            return array('status'=>-1,'msg'=>'添加失败','result'=>'');
        return array('status'=>1,'msg'=>'添加成功','result'=>$address_id);
    }
     ...
     * @param $address_id
     */
    public function set_defaultapp($user_id,$address_id){
        M('region3')->where(array('user_id'=>$user_id))->save(array('default'=>0)); //改变以前的默认地址地址状态
        $row = M('region3')->where(array('user_id'=>$user_id,'address_id'=>$address_id))->save(array('default'=>1));
        if(!$row)
            return false;
        return true;
    }
