...
class OrderLogic extends RelationModel
{
     ...
     * 获取供应商订单商品详情
     */
    public function getSuppliersOrderList($suppliers_id,$order='',$start=0,$page_size=20){
        $res = M('order')->join('LEFT JOIN __ORDER_GOODS__ ON __ORDER_GOODS__.order_id=__ORDER__.order_id')->join('LEFT JOIN __GOODS__ ON __GOODS__.goods_id=__ORDER_GOODS__.goods_id')->where('tptc_goods.suppliers_id='.$suppliers_id)->limit("$start,$page_size")->order($order)->select();
        return $res;
